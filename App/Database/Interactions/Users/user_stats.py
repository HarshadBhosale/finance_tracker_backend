...
def userStats(stats_object):
    ...
    for month in months:
        stats[months[month]] = monthly_stats
    
    all_transactions = Transactions.select(fn.date_part("month", Transactions.date).alias("Month"), Transactions.event, Transactions.category, fn.SUM(Transactions.amount).alias("Amount")).where(Transactions.user_id==user_id, fn.date_part("year", Transactions.date) == year, Transactions.status==1).group_by(fn.date_part("month", Transactions.date), Transactions.event, Transactions.category)

    for transaction in all_transactions:
        Event = "Expense" if transaction.event == -1 else "Income"
        stats[months[int(transaction.Month)]][Event][transaction.category] = transaction.Amount
